...
- `出现CROS错误的根本原因是，两台服务器之间共享XHR对象是不安全的`
- `同源， 共享XHR对象；不同源不能共享XHR对象    而超链接form表单是改变地址栏发请求的,内存中没有对象,数据共享安全 `
...
- 注意：jsonp解决跨域的时候，只支持GET请求(问号传参数)。不支持post请求。
- script的src会发送get请求（可跨域/？参数），响应的内容(`fun({"name":"11"})`)会被当成js代码
- 存在的问题
  - 因为script一打开页面就会执行，能不能点击完一个按钮后再加载script
    - 利用事件，新建script-设置src属性-添加到某一个节点下
- 初步使用jsonp：写死的response.getWriter("sayHello( {"name" : "jack"}))
